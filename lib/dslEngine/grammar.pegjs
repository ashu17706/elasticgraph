...
iterate
  = 'iterate over' index:word where:where? as:as? space* '.'? space* from:skipN? space* size:getN? space* flushAtEnd:'Flush at end'? '.'? space* wait:('Wait for' space+ [0-9]+ space+ 'millis')? '.'?{
    return {
