...
const executeDeep = (parsedInstructions, ctx, es) => {
  ...
  return async.eachSeries(parsedInstructions, (instruction) => {
    ...
    .then((res) => {
      if (res instanceof es.Cache) { //The instruction mutates the ctx most probably to generate a new ctx object
        ctx = res
      }
      return mainInstruction.as && ctx.get(mainInstruction.as) || res
    })
  })
}
...
const createAndSetInSearchResponse = (cache, type, body, queryToEs, searchRes) => {
  return create(cache.es, type, body, cache)
 ...
 */
const create = (es, type, entity, ctx) => {
  ...
  entityBody = _.omit(entityBody, ['_id', '_type'])
  ...
  return es.deep.index({
    ...
    body: sanitizedEntityBody
  }, ctx)
  .catch((err) => {
    debug('Error in indexing document in es', err, 'type', type, 'original document', entity, 'sanitized and tried to save', sanitizedEntityBody)
    throw err
  })
}
...
const executeIndex = (instruction, ctx, es) => {
  ...
  return create(es, type, entity, ctx)
  .then((res) => {
    entity._id = res._id
    return entity
  })
