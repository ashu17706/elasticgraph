...
const create = (es, type, entity) => {
  let entityBody = _.omit(entity._source || entity.fields || entity, ['_id', '_type'])
    ...
    return async.each(items, (item) => {
      ...
      return executeDeep(instruction.childInstructions, newCtx, es)
			.catch((err) => {
				...
				debug('async/each: Exception in executing child instructions', err, 'for instruction', instruction)
				throw err
			})
    })
  }
