...
const executeGet = (instruction, ctx, es) => {
  ...
  const cached = ctx.get(JSON.stringify(query) + 'promise')
  if (cached) {
    return cached
  }

  const returnPromise = es.deep.get(query, ctx).then((getRes) => {
  ...
  ctx.set(JSON.stringify(query) + 'promise', returnPromise)
...
const executeSearch = (instruction, ctx, es) => {
  ...
  esQueryBuilder.generate(type, where, null, {match: true}, (err, query) => {
    ...
    const cached = ctx.get(JSON.stringify(queryToEs) + 'promise')
    if (cached) {
      def.resolve(cached)
    }
    ctx.set(JSON.stringify(queryToEs) + 'promise', def.promise)
