  ...
  .then((res) => {
    ...
    return async.whilst(() => !_.isEmpty(res.hits.hits), () => {
      ...
      numTotalHits += res.hits.hits.length
			debug(instruction.from, numTotalHits)
      const promise = (instruction.from && (numTotalHits < instruction.from) && Q()) || asyncFunctions.each(res.hits.hits, instruction, ctx, es)
