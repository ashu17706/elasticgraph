...
const createIfNeeded = (es, type, entity, ctx) => {
	...
	const alreadyCachedDoc = ctx.getEntity(type, entity._id)
	if (alreadyCachedDoc) {
...
const scrolledSearchAndExecuteChildrenInstructions = (es, searchQuery, ctx, instruction) => {
  let numTotalHits = 0
  let iterationsWithoutFlush = 1 //We will be doing one search anyway

  return es.search(searchQuery) //The first search
	.catch((err) => {
  ...
  .then((res) => {
    ...
    return async.whilst(() => !_.isEmpty(res.hits.hits), () => {
      ...
      let promise
      if (!instruction.from || (numTotalHits >= instruction.from)) {
        promise = asyncFunctions.each(res.hits.hits, instruction, ctx, es)
      }
      promise = promise || Q()
      if (instruction.flushEvery) {
        if (iterationsWithoutFlush === instruction.flushEvery - 1) {
          promise = promise.then(() => {
            return ctx.flush(true).catch(debug)
          })
      ...
      .then(() => {
        ...
        return es.scroll({
          ...
          scroll: instruction.scrollDuration || '180s'
        })
				.catch((err) => {
