...
const Cache = class Cache {
  ...
  **/
  ...
  flush(doDeepFlush) {
		...
		//if (doDeepFlush)
		//	debug(_.keys(this.data).length, _(this.data).keys().sample(50).value())
    ...
    return async.each(updatedEntities, (updatedEntity) => {
      ...
      .catch((e) => debug('Error in indexing entity. Deep flush is ', doDeepFlush, 'updatedEntity is', updatedEntity, 'Error is', e))
    })
		.then(() => {
			if (doDeepFlush) {
				return this.flush()
