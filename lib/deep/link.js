'use strict'
const debug = require('debug')('elasticgraph:deep/link')
  ...
  execute(params, cache) {
    ...
    return async.each(entities, (e2) => {
      ...
      .then((isAlreadyDualLinked) => {
        ...
        if (isAlreadyDualLinked) {
          ...
          return utils.getEntity(cache, params.e1._id, params.e1._type)
          .then((e1Loaded) => {
            return utils.getEntity(cache, e2._id, e2._type)
            .then((e2Loaded) => {
              
              return this.dualLink(e1Loaded, e2Loaded, params.e1ToE2Relation, cache, params.isOwn, params.relationPropertyIsSet)
            })
          })
        }
      })
    ...
    .then(() => {
      if (indexEntitiesAtEnd) {
        return cache.flush()
        .catch((e) => debug('Error in flushing cache to database', e))
      } else {
        return Q()
      }
    })
...
if (require.main === module) {
  ...
  es.deep.link({
    e2: {
      _type: 'event',
      _id: 'b9688b62d86580ae54f1b83ef6c5561688aa55ba'
    },
    e1: {
      _type: 'speaker',
      _id: '78cd1b1bc7effdc2d01c973f66f17ec21b98f983'
    },
    e1ToE2Relation: 'events'
  })
  .then(() => {
    return es.dsl.execute('get event b9688b62d86580ae54f1b83ef6c5561688aa55ba')
  })
  .then((res) => {
    debug(JSON.stringify(res[0].get('b9688b62d86580ae54f1b83ef6c5561688aa55baevent')._source.speakers))
  })
  //.then((res) => {debug(JSON.stringify(res.getEntity('event', 'b9688b62d86580ae54f1b83ef6c5561688aa55ba')))})
  .catch(debug)
}
