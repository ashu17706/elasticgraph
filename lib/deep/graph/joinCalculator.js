  ...
  return async.each(_.keys(compulsoryJoins || joinsForEntity), (toJoinFieldName) => {
    ...
    return async.map(toJoinEntities, (toJoinEntity) => {
      ...
      .then((toJoinDoc) => {
				...
				if (!toJoinDoc) {
					throw new Error('No doc found for params ' +  JSON.stringify(getParams) + ' in cache ' + JSON.stringify(cache.getEntity(getParams._type, getParams._id)))
				}
    ...
    .then((toJoinDocs) => {
      ...
      //debug('resolved joins for ' + toJoinFieldName + ' in ' + entity._type, 'joined entity docs', JSON.stringify(toJoinDocs), 'to join entities before resolution', JSON.stringify(toJoinEntities))
  		...
  		toJoinDocs.forEach((doc) => purgeUnwantedProperties(doc, fields, joinsForEntity[toJoinFieldName]))
...
const purgeUnwantedProperties = (doc, fieldsToFetch, joins) => {
	FIELDS_TO_PURGE.forEach((field) => delete doc[field])
	const docBody = doc._source || doc.fields
	if (docBody) {
		_.keys(docBody).forEach((field) => {
			...
			if (_.isArray(fieldValue)) {
				fieldValue.forEach((entry) => {
					if (_.isObject(entry)) {
						purgeUnwantedProperties(entry, null, joinsForField)
					}
				})
