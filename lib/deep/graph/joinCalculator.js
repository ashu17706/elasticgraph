 ...
 *@param {[String]} edgepathToRightNode - It will be a path to right node
 *
 */
let resolveForEntity = function(cache, langs, joinConfigPathOrConfig, entity, compulsoryJoins) {
  ...
  if (!joinsForEntity) {//edgePathToRightNode && _.isEmpty(edgePathToRightNode))) {
    //debug(entity._type, joinConfigPathOrConfig, joinsForEntity)
    return Q(entity)
  }
  ...
  return async.each(_.keys(compulsoryJoins || joinsForEntity), (toJoinFieldName) => {
    ...
    return async.map(toJoinEntities, (toJoinEntity) => {
      ...
      return cache.es.deep.get(getParams)//utils.getEntity(cache, toJoinEntity._id, toJoinEntity._type) //This get will also save the docs in cache //TODO load only required fields
      .then((toJoinDoc) => {
    ...
    .then(() => {
     ...
     // debug(JSON.stringify(entity[toSetProp]), clonedEntityBody, 'dddddddddddddddddddddddddddddd')
      entity[toSetProp] = mergeDeep(entity[toSetProp] || {}, clonedEntityBody)
  ...
  return async.each(updatedFieldJoinIns, (joinInInfo) => {
    ...
    .then((toUpdateDestEntities) => {
      ...
      return async.each(toUpdateDestEntities.entities, (toUpdateDestEntity) => {
        ...
        return loadJoinsOfEntityWithPath(cache, toUpdateDestEntity, destEntityToUpdatedEntityPath).then((toUpdateLoadedDestEntity) => {
...
const loadJoinsOfEntityWithPath = (cache, entity, path) => {
  ...
  let joins = path.reduce((result, info, i) => {
    if (i % 2 === 0) {
      let type = _.flatten([cache.es.config.schema[entity._type][info].type])[0]
      let id = path[i + 1]

      if (type && id) {
        result[info] = {_type: type, _id: id}
      }
    }
