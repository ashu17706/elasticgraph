...
const forEntity = (es, entityType, joinConfigPathOrConfig, langs, dontResolve) => {
  ...
  if (dontResolve) {
    return getJoinPaths(joinConfig, schema)
  }
  ...
  /**return _(getJoinPaths(joinConfig))
    .map(
      (joinPath) => resolvePath(es.config, entityType, langs, joinPath)
    )
    .flatten()
    .value()**/
}
...
const getJoinPaths = (joinConfig, entitySchema) => {
  ...
  const joinPaths = _.transform(joinConfig, (result, value, key) => {
    if (_.isObject(value)) {
  ...
  if (noFieldsInJoin) {
    joinPaths.concat(_.keys(entitySchema))
  }

  //debug(joinPaths, joinConfig)
  return joinPaths
}
