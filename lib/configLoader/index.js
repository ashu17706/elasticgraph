...
const handleDependency = (config, dependentEt, sourceEt, dependency) => {
  ...
  //debug('extended ' + dependentEt + ' schema with following fields of ' + sourceEt, _.keys(config.schema[sourceEt]), 'Existing fields', _.keys(config.schema[dependentEt]))
  ...
  _.each(config.schema[sourceEt], (sourceFieldSchema, sourceEtField) => {
    if (sourceFieldSchema.isRelationship) {
      ...
      debug(dependentEt, replacementFieldName, sourceFieldSchema.inName)
