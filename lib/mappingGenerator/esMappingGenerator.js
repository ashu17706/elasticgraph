...
var es = require('../app/es')
var configs = es.config
var aggs = require('./setAggregations')


module.exports = function(entityType) {

  var entitySchema = configs.schema[entityType]
  var entityLanguages = configs.common.supportedLanguages
  ...
  _.each(fields, function(field) {
    ...
    if (entitySchema[field].multiLingual && !entitySchema[field].isRelationship) {
      ...
      if (entitySchema[field].autoSuggestion) {
        ...
        let suggestionPath = _.split(entitySchema[field].suggestionPath, '.')
...
if (require.main === module) {
  ...
  var toRecreateIndices = process.argv[4] && process.argv[4].split(',') || _.keys(configs.schema)
  var mappingsOfEntities = _.reduce(toRecreateIndices, function(result, entityType) {
