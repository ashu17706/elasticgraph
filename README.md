...
Graph is part of the name, because here we see every dataset as a Graph of different types of nodes (entities) connected through different relationships. You can read more about Graph databases [here](https://en.wikipedia.org/wiki/Graph_database). While this being said, *EG does not strive to be a full functional Graph database. It simply allows you to configure and execute some graph queries, using ES query API.* It does not support transactions or guarantee ACID behavior. It is designed to give great performance for *write less and read/search/analyse more* scenarios, under heavy query load over big data.



## **Packaging**

ElasticGraph will soon be available as a NodeJS module. A REST server will also be available. Use it as it suits you. 
...
> The simple fields of an entity are defined in
...
The format for specifying relationships in relationship file is
    ...
    relationNameFromAToB <> relationNameFromBToAs
...
* Setup the EG configurations folder
* Install Nodejs with EpicGraph npm

* Generate the ES mappings
...
Built with love from the Himalayas. <3
